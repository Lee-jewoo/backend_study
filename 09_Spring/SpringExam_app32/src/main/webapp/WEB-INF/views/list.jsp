	...
	<div class="container">
		...
		<table class="table table-striped">
			...
			<tbody>
			   ...
			   <tr>
				<td colspan="6">
					<c:set var="totalPage" value="${pagedto.totalRecord/pagedto.perPage}"/>
					<c:if test="${pagedto.totalRecord % pagedto.perPage != 0}">
						<c:set var="totalPage" value="${totalPage+1}"/>
					</c:if>
					<c:forEach var="i" begin="1" end="${totalPage}">
                		<c:if test="${pagedto.curPage==i}">${i}</c:if>
                		<c:if test="${pagedto.curPage!=i}">
